...
def query_index(index, prompt: str, top_k: int):
...
def query(question_idx: int, question_prompt: str, index, model, W: int, K: int) -> str:
    ...
    prompt += question_prompt+'\n'
    rag_results = query_index(index, prompt, K)
    # RAG 
...
def evaluate_questions(index, model, W: int, K: int):
    ...
    # question_text = [voice2text.v2text("record.wav")]
    # question_text = [voice2text.v2text("record.wav"), use_micro = True]
    question_text = ['For the mobility evaluation parameters, what is the agreed value for the TimeToTrigger parameter?']
    print(f"question_text: {question_text[0]}")
    # Initialize counters
    ...
    for question_idx, question_info in tqdm.tqdm(enumerate(question_text)):
        returned_answer = query(question_idx, question_info, index, model, W, K)
...
if __name__ == "__main__":
    ...
    evaluate_questions(index, args.model, W, K)
